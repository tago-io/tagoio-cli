...
async function accessExport(account: Account, import_account: Account, export_holder: IExportHolder) {
  ...
  const list = await account.accessManagement.list({ amount: 10000, fields: ["id", "name", "tags"], filter: { tags: [{ key: export_holder.config.export_tag }] } });
  // @ts-expect-error we are looking only for keys
  const import_list = await import_account.accessManagement.list({ amount: 10000, fields: ["id", "tags"], filter: { tags: [{ key: export_holder.config.export_tag }] } });
